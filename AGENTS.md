...
- Booking emails send to `sup@rootsinbluestone.com` from `SITE.bookingEmail`; delivery prefers Resend with `RESEND_API_KEY` plus a verified `BOOKING_FROM_EMAIL`, and can fall back to Mailchimp Transactional with `MAILCHIMP_TRANSACTIONAL_API_KEY` or `MANDRILL_API_KEY`. The form returns a visible 503 fallback message when no sender is configured. Never commit secrets.
