...
- Booking date-conflict checks must use the website tour-date source of truth in `lib/tour.ts`, not a separate Bandsintown fetch. When the Bandsintown listener adds shows to the site, the booking form should automatically reflect those checked-in dates.
- Booking emails send to `rootsinbluestone@gmail.com` from `SITE.bookingEmail`; delivery prefers Resend with `RESEND_API_KEY` plus a verified `BOOKING_FROM_EMAIL`, and can fall back to Mailchimp Transactional with `MAILCHIMP_TRANSACTIONAL_API_KEY` or `MANDRILL_API_KEY`. The form returns a visible 503 fallback message when no sender is configured. Never commit secrets.
