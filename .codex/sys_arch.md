...
- Resend, with optional Mailchimp Transactional fallback, for booking-email delivery
- Mailchimp Marketing API for newsletter signup
- Printful Quick Store for hosted merch catalog, cart, checkout, payment, tax, shipping, and fulfillment

## Structure
- `app/page.tsx` — single-scroll home (nav → hero → tour → music → about → gallery → video → booking → merch → newsletter → footer)
- `app/book/page.tsx` — standalone booking page
- `app/api/book/route.ts` — edge route; validates with `lib/booking-schema.ts`, emails through Resend when `RESEND_API_KEY` plus a verified `BOOKING_FROM_EMAIL` are configured, can fall back to Mailchimp Transactional, returns HTTP 503 with the direct booking email when delivery is unconfigured, and exposes a non-secret `GET` health response for booking-email config state
...
- `lib/merch.ts` — reviewed public merch selection, final prices, local mockup paths, and direct `ribs.printful.me` product URLs
...
## Configuration
- Env (booking email): booking submissions are sent to `sup@rootsinbluestone.com` from `SITE.bookingEmail`. The preferred sender is Resend via `RESEND_API_KEY`, plus `BOOKING_FROM_EMAIL` from a verified sender/domain. Mailchimp Transactional via `MAILCHIMP_TRANSACTIONAL_API_KEY` or `MANDRILL_API_KEY` remains a fallback. If no provider or verified sender is configured, `/api/book` returns HTTP 503 instead of pretending delivery succeeded. Mailchimp Marketing (`MAILCHIMP_API_KEY`) is only for newsletter audience signups.
...
- Merch checkout: the public `https://ribs.printful.me/` Quick Store hosts the catalog and customer checkout. No Printful API secret is required by the website.
...
- Production direct uploads should be built with `NEXT_PUBLIC_SITE_URL=https://rootsinbluestone.com` so canonical/Open Graph metadata points at the approved apex domain.
- Production custom domains are `rootsinbluestone.com` and `www.rootsinbluestone.com`; the `www` host permanently redirects to the apex. `ribs.music` and `www.ribs.music` are short domains that permanently redirect to the same apex while preserving path and query data.
- Pages secrets needed for real emails/signups: `RESEND_API_KEY`, verified `BOOKING_FROM_EMAIL`, `MAILCHIMP_API_KEY`, and `MAILCHIMP_AUDIENCE_ID`. Mailchimp Transactional keys remain optional fallback configuration. Project compat: `nodejs_compat`.
...
- Reviewed merch mockups live under `public/img/merch/`; the current T-shirt mockup is versioned under `printful-2026-07-20-v2/`, while the hoodie and cap retain their reviewed originals.
