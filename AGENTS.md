...
- Booking emails send to `rootsinbluestone@gmail.com` from `SITE.bookingEmail`; delivery needs `RESEND_API_KEY` and optionally `BOOKING_FROM_EMAIL`. The form degrades gracefully without Resend. Never commit secrets.
