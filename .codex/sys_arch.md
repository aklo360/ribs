...
- `components/site/*` — page sections; `components/booking/booking-form.tsx` — 4-step form. The live quote panel is intentionally hidden until the user reaches the Performance step and chooses a lineup, so the form does not open with the default public Duo estimate. Booked-date conflict warnings render inside that quote panel under the info-icon estimate note and must use `lib/tour.ts` as the same source of truth as the public tour list.
