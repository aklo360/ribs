...
- `components/site/*` — page sections; `components/booking/booking-form.tsx` — 4-step form. The live quote panel is intentionally hidden until the user reaches the Performance step and chooses a lineup, so the form does not open with the default public Duo estimate. Booked-date conflict warnings render immediately under the Event Date field on the Event step and must use `lib/tour.ts` as the same source of truth as the public tour list. The final form step only keeps the open-ended notes textarea; budget, travel/lodging, dress-requirement, and heard-about-us controls are intentionally not part of the visible booking flow.
